...
class IsCustomer(BasePermission):
    """
    Allows access only if the JWT token has 'is_customer' set to True.
    """
    def has_permission(self, request, view):
        ...
        if isinstance(token_payload, dict):
            return token_payload.get('is_customer', False)
...
class IsAccountOfficer(BasePermission):
    """
    Allows access only if the JWT token has 'is_account_officer' set to True.
    """
    def has_permission(self, request, view):
        # 1. Check if user is authenticated at all
        if not request.user or not request.user.is_authenticated:
...
class IsIT(BasePermission):
    """
    Allows access only if the JWT token has 'is_IT' set to True.
    """
    def has_permission(self, request, view):
        # 1. Check if user is authenticated at all
        if not request.user or not request.user.is_authenticated:
